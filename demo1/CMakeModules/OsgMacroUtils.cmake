...
MACRO(LINK_CORELIB_DEFAULT CORELIB_NAME)
    ...
    IF(OSGEDITOR_SONAMES)
      SET_TARGET_PROPERTIES(${CORELIB_NAME} PROPERTIES VERSION ${OSGEDITOR_VERSION} SOVERSION ${OSGEDITOR_SOVERSION})
    ENDIF(OSGEDITOR_SONAMES)
...
MACRO(SETUP_LIBRARY LIB_NAME)
    ...
    ADD_LIBRARY(${LIB_NAME}
        ${OSGEDITOR_USER_DEFINED_DYNAMIC_OR_STATIC}
        ${TARGET_H}
...
MACRO(SETUP_PLUGIN PLUGIN_NAME)
    ...
    SET(TARGET_SRC ${TARGET_SRC} ${OSGEDITOR_VERSIONINFO_RC})
...
MACRO(SETUP_EXE IS_COMMANDLINE_APP)
    ...
    ELSE(${IS_COMMANDLINE_APP})
        ...
        IF(APPLE)
            # SET(MACOSX_BUNDLE_LONG_VERSION_STRING "${OSGEDITOR_MAJOR_VERSION}.${OSGEDITOR_MINOR_VERSION}.${OSGEDITOR_PATCH_VERSION}")
            # Short Version is the "marketing version". It is the version
            # the user sees in an information panel.
            SET(MACOSX_BUNDLE_SHORT_VERSION_STRING "${OSGEDITOR_MAJOR_VERSION}.${OSGEDITOR_MINOR_VERSION}.${OSGEDITOR_PATCH_VERSION}")
            # Bundle version is the version the OS looks at.
            SET(MACOSX_BUNDLE_BUNDLE_VERSION "${OSGEDITOR_MAJOR_VERSION}.${OSGEDITOR_MINOR_VERSION}.${OSGEDITOR_PATCH_VERSION}")
...
MACRO(SETUP_EXAMPLE EXAMPLE_NAME)
        ...
        IF(APPLE)
            INSTALL(TARGETS ${TARGET_TARGETNAME} RUNTIME DESTINATION share/bin BUNDLE DESTINATION share/bin )
        ELSE(APPLE)
            INSTALL(TARGETS ${TARGET_TARGETNAME} RUNTIME DESTINATION share/bin COMPONENT examples )
            IF(MSVC AND NOT CMAKE_BUILD_TYPE STREQUAL "Release")
...
MACRO(HANDLE_MSVC_DLL)
        ...
        #it is hardcoded, we should compute OUTPUT_BINDIR position relative to OUTPUT_LIBDIR ... to be implemented
        #changing bin to something else breaks this hack
        #the dll are versioned by prefixing the name with osg${OSGEDITOR_SOVERSION}-
        ...
        ELSE(${ARGC} GREATER 1)
                SET(LIB_SOVERSION ${OSGEDITOR_SOVERSION})
        ENDIF(${ARGC} GREATER 1)
...
#     SET_TARGET_PROPERTIES(${LIB_NAME} PROPERTIES PREFIX "../../bin/osg${OSGEDITOR_SOVERSION}-")
